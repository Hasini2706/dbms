...
SELECT d.location, COUNT(s.s_id) AS student_count
FROM students s
JOIN departments d
ON s.department_id = d.d_id
GROUP BY d.location;


SELECT c.c_name,SUM(c.price)
FROM courses c
JOIN enrollments e
...
SELECT c.c_name, COUNT(e.student_id) AS enrolled_students
FROM courses c
JOIN enrollments e
ON c.c_id = e.course_id
GROUP BY c.c_name
HAVING COUNT(e.student_id) > 40;

--how many students doesnt have any enrollments
SELECT COUNT(*)
FROM students s
JOIN enrollments e 
ON s.s_id = e.student_id
WHERE s.s_id NOT IN (SELECT student_id FROM enrollments);

SELECT COUNT(*)
FROM faculty f
JOIN course_mappings c
ON f.f_id = c.faculty_id
WHERE f.department_id=105 AND c.role=(SELECT role FROM course_mappings);

SELECT f.f_name
FROM faculty f
JOIN departments d
ON f.f_name = d.hod_name
WHERE dob=(SELECT MAX(dob) FROM faculty);

SELECT f.f_name FROM faculty f
WHERE dob=(SELECT MIN(dob) FROM faculty);

SELECT COUNT(*) FROM enrollments e
WHERE enrolment_date BETWEEN TO_DATE('2025-08-01','YYYY-MM-DD') AND TO_DATE('2025-08-31','YYYY-MM-DD');

SELECT co.c_name
FROM course_mappings c
JOIN courses co
ON c.course_id = co.c_id
WHERE c.role='Secondary' AND c.course_id NOT IN (SELECT course_id FROM enrollments GROUP BY course_id);

SELECT c_id,role
FROM enrollments 
JOIN course_mappings c
ON e.course_id = c.course_id
WHERE role='Secondary' AND course_id NOT IN (SELECT course_id FROM enrollments GROUP BY course_id);
...
from 
courses c
join course_mappings cm
on c.c_id = cm.course_id
where cm.role='Secondary';
...
SELECT f_name
from faculty where department_id=108 and salary=(select max(salary) from faculty where department_id=108);
...
JOIN enrollments e 
ON c.c_id = e.course_id
WHERE c.c_id NOT IN (SELECT course_id FROM enrollments);
...
SELECT c_name from courses where c_id=205;



SELECT count(*)
from enrollments
where enrolment_date BETWEEN TO_DATE('2025-01-01','YYYY-MM-DD') AND TO_DATE('2025-12-31','YYYY-MM-DD');
...
SELECT f_id from faculty where f_name='Shalini Gupta';

select course_id from course_mappings where faculty_id=39;
...
select name from students where dob=(select min(dob) from students);

select d_name,location from departments
group by d_name,location
order by location;
...
--what are the courses with zero enrolments
SELECT c.c_name FROM courses c
WHERE c.c_id NOT IN (SELECT course_id FROM enrollments GROUP BY course_id); 
...
SELECT d.d_name, COUNT(f.f_id) AS faculty_count    
FROM departments d
LEFT JOIN faculty f ON d.d_id = f.department_id 
GROUP BY d.d_name
ORDER BY faculty_count ASC  ;


---in which month highest number of faculty were hired
SELECT TO_CHAR(doj, 'Month') AS hire_month, COUNT(*) AS faculty_count
FROM faculty
...
---how many students studying under faculty named lakshmi rithu across all courses she mapped to either primary or secondary
SELECT COUNT(DISTINCT e.student_id) AS student_count
FROM enrollments e  
JOIN course_mappings cm ON e.course_id = cm.course_id
JOIN faculty f ON cm.faculty_id = f.f_id
WHERE f.f_name = 'Lakshmi Ritu';


--which of the following students havent enrolled for any courses
SELECT s.s_id, s.name
FROM students s
LEFT JOIN enrollments e ON s.s_id = e.student_id
WHERE e.student_id IS NULL;


----dosent belong to list of students with highest number of enrollments
SELECT s.s_id, s.name, COUNT(e.course_id) AS enrollment_count   
FROM students s
LEFT JOIN enrollments e ON s.s_id = e.student_id
