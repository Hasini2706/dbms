...
SELECT question_id, COUNT(*) AS submission_count FROM submissions GROUP BY question_id ORDER BY question_id DESC;

SELECT question_id,COUNT(*) AS submission_count FROM submissions WHERE status = 1 GROUP BY question_id ORDER BY question_id DESC;

SELECT q.q_id,q.q_name,q.rating, COUNT(*) 
FROM submissions s
JOIN question q
ON s.question_id = q.q_id
GROUP BY q.q_id, q.q_name, q.rating;

SELECT q.contest_id, COUNT(*) AS submission_count
FROM submissions s
JOIN question q
ON s.question_id=q.q_id
GROUP BY q.contest_id;

SELECT q.contest_id, q.q_id, q.q_name, COUNT(*) 
FROM submissions s
JOIN question q
ON s.question_id = q.q_id
GROUP BY q.contest_id, q.q_id, q.q_name;

SELECT c.c_name,c.c_date,COUNT(*)
FROM submissions s
JOIN question q 
ON s.question_id=q.q_id
JOIN contest c
...
SELECT c.c_name,c.c_date,c.c_type,COUNT(*)
FROM 
submissions s
JOIN 
question q
...
JOIN contest c
ON q.contest_id =c.c_id
GROUP BY c.c_name,c.c_date,c.c_type;
...
SELECT u.name,c.c_name,q.q_name,COUNT(*)
FROM users u
JOIN submissions s
ON u.u_id = s.user_id
JOIN question q
