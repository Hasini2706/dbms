...
WITH hr_employees AS (
    SELECT id,name,department FROM employee WHERE department='Operations'
)
SELECT department,COUNT(*)
FROM hr_employees
GROUP BY department;

--which department has highest no of employees
WITH cte AS (
...
SELECT department,no_of_emp FROM cte WHERE no_of_emp = (SELECT MAX(no_of_emp) FROM cte);
...
SELECT id,name,salary FROM hr_employees WHERE salary>(SELECT MAX(salary) FROM testing_employees);


SELECT id,name,salary, DENSE_RANK() OVER(ORDER BY salary DESC) AS rnk
FROM employee 
WHERE salary IS NOT NULL;

SELECT id,name,department,gender,salary,RANK() OVER(PARTITION BY department,gender
...
SELECT name,date_of_joining FROM cte WHERE running_sal>1000000
