...
SELECT id,name,date_of_joining,gender FROM employee WHERE gender='Female' AND department='Operations';
SELECT name,salary,department,gender FROM employee WHERE gender='Male' AND (department='SW' OR department='Testing') AND salary>50000;
SELECT id,name,email,date_of_joining FROM employee WHERE date_of_joining IS NULL;
SELECT *FROM employee WHERE department='SW' AND date_of_joining>'01-01-2020';
SELECT id,name,email,department FROM employee WHERE email IS NULL AND department IS NULL;
SELECT id,name,gender,department,salary,date_of_joining FROM employee WHERE gender='Male' AND department='Operations' AND salary<=40000 AND date_of_joining<'15-07-2021';
...
SELECT *FROM employee ORDER BY date_of_joining DESC NULLS LAST;
SELECT *FROM employee ORDER BY department;
SELECT id,name,salary,age,department
FROM employee ORDER BY salary DESC,age DESC,department DESC;

SELECT sum(salary) fROM employee WHERE department='SWE';

SELECT COUNT(age) FROM employee;
SELECT COUNT(gender) FROM employee WHERE department='Operations' AND gender='Female';
...
SELECT MAX(salary)-MIN(salary) AS min_salary FROM employee;
SELECT MAX(date_of_joining) FROM employee;
SELECT MAX(date_of_joining) AS latest_date,MIN(date_of_joining) AS earliest_date FROM employee;

CREATE OR REPLACE PROCEDURE updSalary(emp_id IN NUMBER,new_salary IN NUMBER) IS 
BEGIN 
...
CREATE OR REPLACE deleteAnEmployee(emp_id IN NUMBER) IS
BEGIN 
...
CREATE OR REPLACE PROCEDURE getMinMaxSalaries(dept_name IN VARCHAR2) IS
 min_salary NUMBER;
 max_salary NUMBER;
BEGIN
SELECT MIN(salary),MAX(salary) INTO min_salary,max_salary
...
SELECT SUM(CASE WHEN gender ='Male' THEN salary END) AS male_sal FROM employee;

SELECT 
(SELECT SUM(salary) FROM employee WHERE gender='Male')-
(SELECT SUM(salary) FROM employee WHERE gender='Female') FROM DUAL;
...
CREATE OR REPLACE TRIGGER sal_change_trigger
...
UPDATE employee SET salary=80000 WHERE id=113;

UPDATE employee SET salary=salary+3 WHERE department='Testing' AND salary IS NOT NULL;
...
--Non-Updatable View because derived columns are present 
SELECT department,COUNT(*) FROM employee 
GROUP BY department ;

CREATE OR REPLACE VIEW dept_counts AS 
SELECT department,COUNT(*) AS cnt
FROM employee 
GROUP BY department;

UPDATE dept_counts SET cnt=26 WHERE department='HR';--fails because we cant change count directly
SELECT *FROM dept_counts;

--MATERIALIZED VIEW
--a view that actually stores the data
--data in the disk 
--it always recomputes the result 
--freshly from underlying tables
--rather it stores them premanently
--if you want fresh data to be fetched 
--a manual refresh (using DBMS_MVIEW.REFRESH()) is required
CREATE MATERIALIZED VIEW dept_emp_counts AS
SELECT department,COUNT(*) AS cnt
FROM employee 
GROUP BY department;

DELETE FROM employee WHERE id = 117;

SELECT *FROM dept_counts;  --data from nrml view

SELECT *FROM dept_emp_counts;--materialized view

--To refresh the mat view
BEGIN
